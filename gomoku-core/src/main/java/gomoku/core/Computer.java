...
//    vector<int> nextMoveAlphaBeta(BoardC board) {
//        vector<int> v = {0, 0};
//        int M = MAX_VALUE, x = -1, y = -1;
//
//        for (int i = 0; i <= board.N - 1; i++) {
//            for (int j = 0; j <= board.N - 1; j++) {
//                if (board.getValue(i, j) == '.' && adjacentPlaced(board, i, j)) {
//                    int temp = minimaxAlphaBeta(board, 2, true, MIN_VALUE, MAX_VALUE, i, j);
//                    if (M < temp) {
//                        M = temp;
//                        x = i;
//                        y = j;
...
//    int minimaxAlphaBeta(BoardC board, int depth, boolean isMax, int alpha, int beta, int x, int y) {
...
//                return MAX_VALUE;
//            } else {
//                return MIN_VALUE;// player wins, therefore, need to minimize score so computer will not choose this path
//            }
...
//
//
//    int evaluation(BoardC board, boolean isMax) {//if isMax is true, computer is about to make the move at (x,y)
//
//        int sum = 0;
...
//                    boolean needMax = c == board.getComputerSymbol();
//
//
...
//                    int l = 1;
//                    while (i + l <= board.N - 1 && board.getValue(i + l, j) == c) {
//                        sameSymbol++;
//                        l++;
//                    }
//
...
//                    //consider value at i - k later to see if it's blocked or not
//                    l = 1;
//                    while (j + l <= board.N - 1 && board.getValue(i, j + l) == c) {
//                        sameSymbol++;
//                        l++;
//                    }
//
...
//                    //consider value at i - k later to see if it's blocked or not
//                    l = 1;
//                    while (i + l <= board.N - 1 && j + l <= board.N - 1 && board.getValue(i + l, j + l) == c) {
//                        sameSymbol++;
//                        l++;
//                    }
//
...
//                    k = 1;
//                    while (i - k >= 0 && j + k <= board.N - 1 && board.getValue(i - k, j + k) == c) {
//                        sameSymbol++;
//                        k++;
//                    }
//
...
//                    //consider value at i - k later to see if it's blocked or not
//                    l = 1;
//                    while (i + l <= board.N - 1 && j - l >= 0 && board.getValue(i + l, j - l) == c) {
//                        sameSymbol++;
//                        l++;
//                    }
//
...
//            }
//        }
//        if (computerPattern[M] > 0) return MAX_VALUE;
//        if (playerPattern[M] > 0) return MIN_VALUE;
//        int x = 1;
...
//
//
//    boolean adjacentPlaced(BoardC board, int x, int y) {
//
//        boolean value = false;
...
//        for (auto d : adjacent) {
//
//            if (x + d[0] >= 0 && y + d[1] >= 0 && x + d[0] <= board.N - 1 && y + d[1] <= board.N - 1) {
...
//    boolean checkVisitedBoard(BoardC board) {
...
//    int getEvaluation(BoardC board) {
...
//
//    void insertToHashMap(BoardC board, int eval) {
//        String s = board.toString();
//        hashMap[s] = eval;
//        return;
//    }
